...
RSpec.shared_examples 'name searchable concern' do |factory_name|
  ...
  it 'Found records with expression in :name' do
    ...
    expect(found_records.to_a).to match_array(records_to_find)
  end

  it 'igonores records without expression in :name' do
    found_records = described_class.search_by_name(search_params)
    expect(found_records.to_a).to_not match_array(records_to_ignore)
  end
end
